...
### 包头版本兼容性
当前 wire 协议只支持 `GTP_VERSION`。发包统一通过 `GtpMakePacketVersion()` 写版本，收包统一通过 `GtpIsSupportedPacketVersion()` 校验版本，按 `GtpPacketUsesChangeZone()` 判断当前版本是否携带 change zone。后续要兼容新版本时，只扩展这些 helper，不在热路径散落版本判断。
