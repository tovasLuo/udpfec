...
#include <regex.h>
#endif
#if defined(LIBCLI_USE_POLL) && !defined(_WIN32)
#include <poll.h>
...
ssize_t READ(SOCKET fd, void *buf, size_t count) {
  return (ssize_t)recv(fd, (char*)buf, (int)count, 0);
}

ssize_t WRITE(SOCKET fd, const void *buf, size_t count) {
  return (ssize_t)send(fd, (const char*)buf, (int)count, 0);
}
#else
#define READ read
#define WRITE write
...
static int cli_count_filter_init(struct cli_def* cli, int argc, UNUSED(char **argv), struct cli_filter *filt);
static int cli_match_filter(UNUSED(struct cli_def *cli), const char *string, void *data);
static int cli_range_filter(UNUSED(struct cli_def *cli), const char *string, void *data);
...
static int cli_int_unregister_buildmode_command(struct cli_def *cli, const char *command) ATTRIBUTE((unused));
...
#if 0
static void cli_int_show_pipeline(struct cli_def *cli, struct cli_pipeline *pipeline);
#endif
...
static int cli_socket_wait(SOCKET sockfd, struct timeval *tm);
...
static ssize_t write_block(SOCKET fd, const void *buf, size_t count) {
  size_t written = 0;
  ssize_t thisTime = 0;
  while (count != written) {
...
static int cli_add_history(struct cli_def *cli, const char *cmd) {
  ...
  free(cli->history[0]);
  for (int i = 0; i < MAX_HISTORY - 1; ++i) cli->history[i] = cli->history[i + 1];
...
static char *cli_int_return_newword(const char *start, const char *end) {
  ...
  if (!(newword = calloc((size_t)len + 1, 1))) return 0;
  to = newword;
  while (start != end) {
    if (*start == '\\')
      start++;
...
static int cli_parse_line(const char *line, char *words[], int max_words) {
  ...
  while (nwords < max_words - 1) {
    ...
    if (p && (!*p || *p == inquote || (word_start && !inquote && (isspace(*p) || *p == '|')))) {
...
void cli_get_completions(struct cli_def *cli, const char *command, char lastchar, struct cli_comphelp *comphelp) {
  ...
  struct cli_pipeline_stage *stage = NULL;
...
static ssize_t show_prompt(struct cli_def *cli, SOCKET sockfd) {
  ssize_t len = 0;

  if (cli->hostname) len += write_block(sockfd, cli->hostname, strlen(cli->hostname));

  if (cli->modestring) len += write_block(sockfd, cli->modestring, strlen(cli->modestring));
  if (cli->buildmode) {
    len += write_block(sockfd, "[", 1);
    len += write_block(sockfd, cli->buildmode->cname, strlen(cli->buildmode->cname));
    len += write_block(sockfd, "...", 3);
    if (cli->buildmode->mode_text) len += write_block(sockfd, cli->buildmode->mode_text, strlen(cli->buildmode->mode_text));
    len += write_block(sockfd, "]", 1);
  }
  return len + write_block(sockfd, cli->promptchar, strlen(cli->promptchar));
}

int cli_loop(struct cli_def *cli, SOCKET sockfd) {
  int is_telnet_option = 0, skip = 0;
  size_t l = 0, oldl = 0, cursor = 0;
  ssize_t n = 0;
  char esc = 0;
  ...
  if (cli->telnet_protocol) {
    static const char *negotiate =
        ...
        "\xFF\xFD\x03"
        "\xFF\xFD\x01";
    write_block(sockfd, negotiate, strlen(negotiate));
  }
  ...
  int fd = _open_osfhandle((intptr_t)sockfd, 0);
  if (fd == -1) {
    free(cmd);
    closesocket(sockfd);
    return CLI_ERROR;
  }
  if (!(cli->client = FDOPEN(fd, "w+"))) {
    _close(fd);
#else
  ...
  while (1) {
    ...
    while (1) {
      ...
      if (cli->showprompt) {
        if (cli->state != STATE_PASSWORD && cli->state != STATE_ENABLE_PASSWORD) write_block(sockfd, "\r\n", 2);

        switch (cli->state) {
          case STATE_LOGIN:
            write_block(sockfd, "Username: ", strlen("Username: "));
            break;

          case STATE_PASSWORD:
            write_block(sockfd, "Password: ", strlen("Password: "));
            break;
          ...
          case STATE_ENABLE:
            ...
            if (cursor < l) {
              size_t n1 = l - cursor;
              while (n1--) write_block(sockfd, "\b", 1);
            }
            break;
      ...
      if ((unsigned char)c == 255 && !is_telnet_option) {
        ++is_telnet_option;
        continue;
      }

      if (is_telnet_option) {
        if ((unsigned char)c >= 251 && (unsigned char)c <= 254) {
          is_telnet_option = c;
          continue;
        }
      ...
      if (c == CTRL('W') || c == CTRL('H') || c == 0x7f) {
        ...
        if (back) {
          while (back--) {
            if (l == cursor) {
              cmd[--cursor] = 0;
              if (cli->state != STATE_PASSWORD && cli->state != STATE_ENABLE_PASSWORD) write_block(sockfd, "\b \b", 3);
            } else {
              if (cli->state != STATE_PASSWORD && cli->state != STATE_ENABLE_PASSWORD) {
                ...
                write_block(sockfd, "\b", 1);
                write_block(sockfd, cmd + cursor, l - cursor);
                write_block(sockfd, " ", 1);
                ...
                for (size_t i = l; i >= cursor; --i) write_block(sockfd, "\b", 1);
              }
              cursor--;
            }
            l--;
      ...
      if (c == CTRL('L')) {
        ...
        for (size_t i = 0; i < cursorback; ++i) write_block(sockfd, "\b", 1);
      ...
      if (c == CTRL('K')) {
        ...
        if (cli->state != STATE_PASSWORD && cli->state != STATE_ENABLE_PASSWORD) {
          for (size_t cptr = cursor; cptr < l; ++cptr) write_block(sockfd, " ", 1);

          for (size_t cptr = cursor; cptr < l; ++cptr) write_block(sockfd, "\b", 1);
        }
        ...
        if (comphelp.num_entries == 0) {
          ...
          for (i = 0; i < comphelp.num_entries; i++) {
            if (i % 4 == 0)
              ...
              write_block(sockfd, " ", 1);
            write_block(sockfd, comphelp.entries[i], strlen(comphelp.entries[i]));
          }
          write_block(sockfd, "\r\n", 2);
          cli->showprompt = 1;
          ...
          if (comphelp.entries[0][0] != '[' && comphelp.entries[0][0] != '<') {
            ...
            cmd[l++] = ' ';
            cursor = l;
            write_block(sockfd, comphelp.entries[0], strlen(comphelp.entries[0]));
            write_block(sockfd, " ", 1);
          ...
          for (size_t i = 1; k != 0 && (int)i < comphelp.num_entries; i++) {
      ...
      if (c == CTRL('B') || c == CTRL('F')) {
        if (c == CTRL('B')) {
          // Left
          if (cursor) {
            if (cli->state != STATE_PASSWORD && cli->state != STATE_ENABLE_PASSWORD) write_block(sockfd, "\b", 1);
          ...
          // Right
          if (cursor < l) {
            if (cli->state != STATE_PASSWORD && cli->state != STATE_ENABLE_PASSWORD) write_block(sockfd, &cmd[cursor], 1);
      ...
      if (cursor == l) {
        ...
        for (int i = 0; i < (int)(l - cursor); ++i) write_block(sockfd, "\b", 1);
        cursor++;
      }
...
static void _print(struct cli_def *cli, int print_mode, const char *format, va_list ap) {
  ...
  size_t n = (size_t)vasprintf(&p, format, ap);
...
struct cli_command *cli_int_register_buildmode_command(struct cli_def *cli, struct cli_command *parent,
  ...
  c->privilege = privilege;
  c->mode = mode;
  if (help && !(c->help = strndup(help, (size_t)(local_strchrnul(help, '\v') - help)))) {
    free(c->command);
    free(c);
    return NULL;
  }
...
int cli_add_comphelp_entry(struct cli_comphelp *comphelp, const char *entry) {
  ...
  if (comphelp && entry) {
    ...
    char **duparray = (char **)realloc((void *)comphelp->entries, sizeof(char *) * ((size_t)(comphelp->num_entries + 1)));
...
void cli_free_comphelp(struct cli_comphelp *comphelp) {
  if (comphelp) {
    for (int idx = 0; idx < comphelp->num_entries; ++idx) free_z(comphelp->entries[idx]);
...
int cli_int_execute_pipeline(struct cli_def *cli, struct cli_pipeline *pipeline) {
  ...
  while (cli->filters) {
    struct cli_filter *filt1 = cli->filters;
    if (filt1->filter) filt1->filter(cli, NULL, cli->filters->data);
    cli->filters = filt1->next;
    free_z(filt1);
  }
  cli->found_optargs = NULL;
  cli->pipeline = NULL;
...
void cli_int_wrap_help_line(char *nameptr, char *helpptr, struct cli_comphelp *comphelp) {
  size_t maxwidth = 80;  // temporary assumption, to be fixed later when libcli 'understands' screen dimensions
  size_t availwidth;
  size_t namewidth;
  size_t toprint;
  char *crlf;
  ...
  do {
    ...
    namewidth = MAX(MAXWIDTHCOL1, nameptr ? strlen(nameptr) : 0);
    ...
    if ((crlf = strpbrk(helpptr, "\n\r"))) {
      ...
      if ((size_t)(crlf - helpptr) < toprint) {
        // ok, crlf is before the wrap, so have line break here.
        toprint = (size_t)(crlf - helpptr);
      }
    }
...
static int cli_socket_wait(SOCKET sockfd, struct timeval *tm) {
  ...
  FD_ZERO(&r);
  FD_SET(sockfd, &r);
  return select((int)sockfd + 1, &r, NULL, NULL, tm);
#endif
}
#if defined(_MSC_VER)
#pragma warning(pop)
